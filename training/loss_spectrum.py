...
from DiffAugment_pytorch_spectrum import DiffAugment
...
class StyleGAN2Loss(Loss):
    def __init__(self, device, G_mapping, G_synthesis, D, diffaugment='', brightness=1.0, saturation=1.0, contrast=1.0, translation=0.125, cutout=0.5, log_amplitude = 0.2, time_shift = 0.5, freq_mask = 0.2 , augment_pipe=None, style_mixing_prob=0.9, r1_gamma=10, pl_batch_shrink=2, pl_decay=0.01, pl_weight=2, D_augself=1, G_augself=1):
        super().__init__()
        self.device = device
    ...
    def accumulate_gradients(self, phase, real_img, real_c, gen_z, gen_c, sync, gain):
        ...
        if do_Gmain:
            with torch.autograd.profiler.record_function('Gmain_forward'):
                ...
                for aug in filter(None, self.D.augself.split(',')):
                    loss_Gaugself += torch.nn.functional.mse_loss(gen_preds_augself[aug],  gen_labels_augself[aug])
                    loss_Gaugself -= torch.nn.functional.mse_loss(gen_preds_augself[aug], -gen_labels_augself[aug])
        ...
        if do_Dmain:
            with torch.autograd.profiler.record_function('Dgen_forward'):
                ...
                for aug in filter(None, self.D.augself.split(',')):
                    loss_Dgen_augself += torch.nn.functional.mse_loss(gen_preds_augself[aug], -gen_labels_augself[aug])
        ...
        if do_Dmain or do_Dr1:
            ...
            with torch.autograd.profiler.record_function(name + '_forward'):
                ...
                loss_Dreal = 0
                loss_Dreal_augself = 0
                if do_Dmain:
                    loss_Dreal = torch.nn.functional.softplus(-real_logits) # -log(sigmoid(real_logits))
                    training_stats.report('Loss/D/loss', loss_Dgen + loss_Dreal)
                    for aug in filter(None, self.D.augself.split(',')):
                        loss_Dreal_augself += torch.nn.functional.mse_loss(real_preds_augself[aug], real_labels_augself[aug])
