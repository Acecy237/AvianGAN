...
from training import training_spectrum_loop
...
def setup_training_loop_kwargs(
    ...
    # args.training_set_kwargs = dnnlib.EasyDict(class_name='training.dataset_spectrum.ImageFolderDataset', path=data, use_labels=True, max_size=None, xflip=False)
    ...
    args.G_kwargs = dnnlib.EasyDict(class_name='training.networks_spectrum.Generator', z_dim=512, w_dim=512, mapping_kwargs=dnnlib.EasyDict(), synthesis_kwargs=dnnlib.EasyDict())
    args.D_kwargs = dnnlib.EasyDict(class_name='training.networks_spectrum.Discriminator', block_kwargs=dnnlib.EasyDict(), mapping_kwargs=dnnlib.EasyDict(), epilogue_kwargs=dnnlib.EasyDict())
    ...
    args.loss_kwargs = dnnlib.EasyDict(class_name='training.loss_spectrum.StyleGAN2Loss', r1_gamma=spec.gamma)
...
def subprocess_fn(rank, args, temp_dir):
    ...
    training_spectrum_loop.training_loop(rank=rank, **args)
...
# General options.
@click.option('--outdir', help='Where to save the results', default="training-runs", metavar='DIR')
...
# Dataset.
@click.option('--data', help='Training data (directory or zip)', metavar='PATH', default='dataset/Bar-tailedGodwit')
...
@click.option('--kimg', help='Override training duration', type=int, metavar='INT', default=5000)
...
@click.option('--d_augself', help='Weight for AugSelf of D', type=float, default=1)
@click.option('--g_augself', help='Weight for AugSelf of G', type=float, default=1)
